...
| --- | --- | --- | --- |
| [Subagent](extensions/subagent/) | Delegate independent tasks to continuable Pi child sessions. | `pi_subagent` actions: `spawn`, `wait`, `send`, `stop`, `list`. | Separate context, scout/worker tool scopes intersected with parent active built-ins, pinned model/thinking, busy steering, branch-sensitive tool-result checkpoints, four active turns, deadlines and turn budgets. Shared filesystem; no nested delegation or other extension hooks. Completion is collected with `wait`/`list`. Supports TUI, JSON and RPC. |
