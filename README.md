...
| --- | --- | --- | --- |
| [Subagent](extensions/subagent/) | Delegate independent tasks to continuable Pi child sessions. | `pi_subagent` actions: `spawn`, `wait`, `send`, `stop`, `list`. | Separate context, scout/worker tool scopes, pinned model/thinking, busy steering, four active turns, deadlines and turn budgets. Runs, full reports, immutable checkpoints, and artifacts are archived under `~/.pi/subagents/`; parent tool results keep branch-specific references. `--no-session` keeps child state in memory. Shared filesystem; no nested delegation or other extension hooks. Completion is collected with `wait`/`list`. Supports TUI, JSON and RPC. |
