...
export class SubagentStorage {
	...
	private async checkDirectory(ref: ArchiveRef): Promise<string> {
		let path = this.root;
		for (const id of [ref.parentId, ref.childId, ref.runId]) {
	...
	async protect(parentId: string): Promise<void> {
		...
		// Keep those runs safe from cleanup by other Pi processes for our lifetime.
		...
		await atomicJson(join(path, `.lease-${process.pid}.json`), { format: "pi-subagent-lease-v1", pid: process.pid });
	}

	private async hasActiveLease(parentId: string): Promise<boolean> {
		const path = join(this.root, parentId);
		for (const entry of await readdir(path, { withFileTypes: true })) {
			if (!entry.isFile() || !/^\.lease-\d+\.json$/.test(entry.name)) continue;
			try {
				const lease = JSON.parse(await readFile(join(path, entry.name), "utf8"));
				if (lease.format !== "pi-subagent-lease-v1" || !Number.isInteger(lease.pid) || lease.pid <= 0 || entry.name !== `.lease-${lease.pid}.json`) continue;
	...
	async readHistory(ref: ArchiveRef): Promise<History> {
		const file = join(await this.checkDirectory(ref), "checkpoint.json");
		await this.protect(ref.parentId);
		if (!(await lstat(file)).isFile()) throw new Error("Subagent checkpoint is not a regular file.");
		const saved = JSON.parse(await readFile(file, "utf8")) as { version?: number; ref?: ArchiveRef; history?: History };
		if (saved.version !== 1 || saved.ref?.parentId !== ref.parentId || saved.ref.childId !== ref.childId || saved.ref.runId !== ref.runId || !Array.isArray(saved.history)) {
	...
	async prune(now = Date.now(), protectedParents: ReadonlySet<string> = new Set()): Promise<void> {
		...
		const candidates: { path: string; parentId: string; bytes: number; time: number; protected: boolean }[] = [];
			...
			for (const child of await directories(join(this.root, parent.name))) {
				for (const run of await directories(join(this.root, parent.name, child.name))) {
					...
					try {
						...
						if (meta.format !== "pi-subagent-run-v1" || meta.ref?.parentId !== parent.name || meta.ref.childId !== child.name || meta.ref.runId !== run.name || !Number.isFinite(meta.createdAt) || (meta.completedAt !== undefined && !Number.isFinite(meta.completedAt)) || !Number.isInteger(meta.pid) || meta.pid <= 0) continue;
						candidates.push({ path, parentId: parent.name, bytes: await size(path), time: meta.completedAt ?? meta.createdAt, protected: processAlive(meta.pid) || protectedParent });
		...
		for (const entry of candidates.sort((a, b) => a.time - b.time)) {
			...
			if (expired || (this.maxBytes > 0 && total > this.maxBytes)) {
				if (await this.hasActiveLease(entry.parentId)) continue;
...
export class RunArchive {
	...
	async finish(child: Child, fullOutput: string): Promise<void> {
		try { await this.save(child, fullOutput); }
		catch (error) {
			const { history: _history, ...configuration } = child;
	...
	private async save(child: Child, fullOutput: string): Promise<void> {
		await this.pending;
		if (this.failure) throw this.failure;
		await writeFile(join(this.directory, "result.md"), fullOutput, { flag: "wx", mode: 0o600 });
		await writeFile(join(this.directory, "checkpoint.json"), JSON.stringify({ version: 1, ref: this.meta.ref, history: child.history }) + "\n", { flag: "wx", mode: 0o600 });
		const { history: _history, ...configuration } = child;
		this.meta = { ...this.meta, completedAt: Date.now(), child: { ...configuration, checkpoint: this.meta.ref } };
